...
import { callName, callSummary, computeStats, isRunning, READONLY_TOOLS, resultText, shortenPath } from './tool-stats.ts'
...
const ToolEntry = memo(function ToolEntry({
  nodes, turn, cwd, openFile, inspectCall,
}: {
  ...
  return (
    ...
    >
      <span className={`${NS}__entry-icon`} aria-hidden>🔧</span>
      <span className={`${NS}__entry-text`}>
        {running ? `工具 ${stats.total} 进行中` : `工具 ×${stats.total}`}
      </span>
