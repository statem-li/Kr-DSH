 ...
 */
function ReasoningEntry({ items, running, t, turn }: {
  ...
  return (
    <div className="dsh-better-markdown__reasoning-entry" data-running={running || undefined}>
      ...
      >
        <span className="dsh-better-markdown__reasoning-entry-icon" aria-hidden>🧠</span>
        <span>{running ? '思考中…' : `思考 ×${items.length}`}</span>
      </button>
    </div>
  )
}
