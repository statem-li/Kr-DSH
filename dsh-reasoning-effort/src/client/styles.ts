...
.re-effort-trigger:hover,
.re-effort-trigger[aria-expanded="true"] {
  background: var(--dsw-alias-fill-tertiary, rgba(120,125,140,.1));
}
.re-effort-trigger:disabled { cursor: not-allowed; opacity: .5; }
.re-model-name {
  overflow: hidden;
...
.re-manual { padding: 2px 0; }
.re-manual-head { display: flex; align-items: baseline; justify-content: space-between; gap: 8px; }
.re-manual-title { color: var(--dsw-alias-label-primary, #15171b); font-size: 13px; font-weight: 500; }
.re-manual-hint { color: var(--dsw-alias-label-tertiary, #9296a0); font-size: 11px; }
.re-manual-row { display: flex; gap: 8px; margin-top: 8px; }
.re-manual-input {
  flex: 1 1 auto;
  min-width: 0;
  height: 30px;
  ...
  border-radius: 8px;
  color: var(--dsw-alias-label-primary, #15171b);
  background: var(--dsw-alias-bg-page, rgba(120,125,140,.06));
  font: inherit;
  font-size: 12px;
  outline: none;
}
.re-manual-input:focus { border-color: var(--dsw-static-deepseek-500, #4d70ff); }
.re-manual-apply {
  flex: 0 0 auto;
  height: 30px;
  padding: 0 12px;
...
.re-manual-apply:disabled { opacity: .6; cursor: wait; }
.re-manual-current { margin-top: 6px; color: var(--dsw-alias-label-tertiary, #9296a0); font-size: 11px; }
`
